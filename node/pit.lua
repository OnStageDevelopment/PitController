...
local function translate_speed(speed)  --Accounts for the positioning of the motors on the pit lift pulleys to adjust to negative to achieve same rotation direction
...
local function config_reader(config_file)  --Decodes config JSON file and sets variables for program to run
    ...
    node_id = decoded["id"]
    complete_down = decoded["complete_down"]
    complete_up = decoded["complete_up"]
    pit_level = decoded["pit_level"]
    floor_level = decoded["floor_level"]
    stage_level = decoded["stage_level"]
...
local function move(amount,direction,gospeed) -- Function to move the pit lift the correct amount of blocks at speed
    if amount == nil then
        return nil
    end
...
local function pos_file_util(cmd,pos) -- Handles writing the new pit lift position and getting the current pit lift position
...
local function check_custom(custom_pos) --Check if the custom pit lift height is valid
...
while true do
    ...
    if command_logic_process(cmd_res[1]) then
        ...
        if cmd_res[2] == "stage" then
            local calcs = calc_pos(stage_level)
            move(calcs[1],calcs[2],move_speed)
            pos_file_util("set",stage_level)

        elseif cmd_res[2] == "pit" then
            local calcs = calc_pos(pit_level)
            move(calcs[1],calcs[2],move_speed)
            pos_file_util("set",pit_level)

        elseif cmd_res[2] == "floor" then
            local calcs = calc_pos(floor_level)
            move(calcs[1],calcs[2],move_speed)
            pos_file_util("set",floor_level)

        elseif cmd_res[2] == "complete_up" then
            local calcs = calc_pos(complete_up)
            move(calcs[1],calcs[2],move_speed)
            pos_file_util("set",complete_up)

        elseif cmd_res[2] == "complete_down" then
            local calcs = calc_pos(complete_down)
            move(calcs[1],calcs[2],move_speed)
            pos_file_util("set",complete_down)
